...
public class DatabaseOperation {
    ...
    public void addCard(Card card){
        if(checkId(card.getUserId(),"card_id",card.getCardName())){
    ...
    public void addTask(Task task){
        if(checkId(task.getUserId(),"task_id",task.getTaskName())){
    ...
    public void deleteTask(Task task){
        if(!checkId(task.getUserId(),"task_id",task.getTaskName())){
            ...
            SQLiteDatabase db = databaseHelper.getWritableDatabase();
            db.delete(table_name,"user_id = ? and task_id = ?",new String[]{task.getUserId(), String.valueOf(task.getTaskId())});
            db.close();
        }
    }
    ...
    public void updateTask(Task task){
        if(!checkId(task.getUserId(),"task_name",task.getTaskName())){
            addTask(task);
        }else{
            SQLiteDatabase db = databaseHelper.getWritableDatabase();
            ContentValues values = new ContentValues();
            ...
            values.put("info",task.getInfo());
            values.put("task_state",task.getTaskState());
            db.update(table_name,values,"user_id = ? and task_id = ?",new String[]{task.getUserId(), String.valueOf(task.getTaskId())});
            db.close();
        }
    }
    ...
    public boolean checkId(String userid,String itemIdName,String name){
        SQLiteDatabase sdb = databaseHelper.getReadableDatabase();
        String sql = "select * from "+table_name+" where user_id = ? and "+itemIdName+" = ?";
        Cursor cursor = sdb.rawQuery(sql,new String[]{userid, name});
        if(cursor.getCount()>0){
            return true;
        }else{
            return false;
